...
exports.login = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      ...
      await db.execute('UPDATE users SET session_key = ? WHERE openid = ?', [
        session_key,
        openid,
      ]);
...
exports.updateProfile = async (req, res, next) => {
  ...
  if (!nickname || !phone) {
    return res
      .status(400)
      .json({ success: false, message: '昵称和电话不能为空' });
  }
...
exports.decryptPhone = async (req, res, next) => {
  ...
  try {
    ...
    await db.execute('UPDATE users SET phone = ? WHERE id = ?', [
      phoneNumber,
      userId,
    ]);
    ...
    if (err.message.includes('Illegal Buffer')) {
      return res
        .status(401)
        .json({ success: false, message: '解密失败，请重新登录后再试' });
    }
    next(err);
  }
};
