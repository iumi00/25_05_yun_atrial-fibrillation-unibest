...
exports.updateQuestionnaireItem = async (req, res) => {
  try {
    ...
    // 删除旧选项
    await db.execute('DELETE FROM questionnaire_options WHERE item_id = ?', [
      id,
    ]);
...
exports.commitUserQuestionnaire = async (req, res) => {
  try {
    const { userId, questionnaireType, score, answers, submitTime, status } =
      req.body;
  ...
  try {
    ...
    const historyRecords = rows.map((record) => ({
      ...
      questionnaireDisplayName:
        record.questionnaireDisplayName || record.questionnaireType,
      score: record.score,
...
exports.delAnswer = async (req, res) => {
  try {
    ...
    // 先删除详情记录
    await db.execute('DELETE FROM user_answer_details WHERE answer_id = ?', [
      id,
    ]);

    // 再删除主记录
    await db.execute('DELETE FROM user_answers WHERE id = ?', [id]);
